...
class AutoUploadController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        //         $ws = WarehousesStock::where('warehouse_id', $warehouse->id)
        ...
        // $pdps = ProductDillerPrice::get();
        // $arr = [];
        //
        // foreach ($pdps as $key => $pdp) {
        //     if (!in_array($pdp->id, $arr)) {
        //         $arr[$pdp->product_id.$pdp->currency_id] = $pdp->id;
        //     }
        // }

        // ProductDillerPrice::whereNotIn('id', $arr)->delete();
                ...
                foreach ($parameter->parameterValues()->get() as $key => $value) {
                    ...
                    if (is_null($subproduct)) {
                        SubProduct::create([
                            ...
                            'value_id' => $value->id,
                            'code' => $product->code.'-'.$value->translation->name,
            ...
            if ($prodPrice->currency->exchange_dependable == 1) {
                if ($mainPrice->id == $key) {
                    ProductPrice::where('id', $key)->update([
                        ...
                        // 'b2b_old_price' => $pricesB2B[$key] - ($pricesB2B[$key] * $product->discount / 100)
                    ]);
                    $this->regeneratePricesStep2($product);
                }
            }else{
                ...
                if (!is_null($mainProductPrice)) {
                    ...
                    ProductPrice::where('product_id', $product->id)
                                ...
                                ->update([
                                    ...
                                    'b2b_old_price' => $exchangeB2B - ($exchangeB2B * $product->discount / 100)
                                ]);
                }
        }
    }
