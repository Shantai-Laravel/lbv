...
class BlogCategoryController extends Controller
{
    ...
    public function edit($id)
    {
        ...
        return view('admin::admin.blogCategories.edit', compact('category', 'galleries'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $doc = $request->get('old_document') ?? null;

        if ($request->get('on_home') == 'on') { $on_home = 1; }
        if ($request->get('active') == 'on') { $active = 1; }

        if($file = $request->file('icon')){
            $name_image = str_replace(' ', '', uniqid(). '-' .$file->getClientOriginalName());
        ...
        if($file = $request->file('banner_desktop')){
            $banner_desktop = str_replace(' ', '', uniqid(). '-' .$file->getClientOriginalName());
            $image_resize = Image::make($file->getRealPath());

            $image_resize->save(public_path('images/blogCategories/og/' .$banner_desktop), 75);

            if ($blogCategory->banner_desktop) {
                @unlink(public_path('images/blogCategories/og/'.$blogCategory->banner_desktop));
            }
        }

        if($file = $request->file('banner_mobile')){
            $banner_mobile = str_replace(' ', '', uniqid(). '-' .$file->getClientOriginalName());
            $image_resize = Image::make($file->getRealPath());

            $image_resize->save(public_path('images/blogCategories/og/' .$banner_mobile), 75);

            if ($blogCategory->banner_mobile) {
                @unlink(public_path('images/blogCategories/og/'.$blogCategory->banner_mobile));
            }
        }

        if($file = $request->file('document')){
            $doc = str_replace(' ', '', uniqid(). '-' .$file->getClientOriginalName());
            // $image_resize = Image::make($file->getRealPath());
            $file->move(public_path('images/blogCategories/docs/' .$doc), 75);

            if ($blogCategory->doc) {
                @unlink(public_path('images/blogCategories/docs/'.$blogCategory->doc));
            }
        }
        ...
        $blogCategory->banner_desktop = $banner_desktop;
        $blogCategory->banner_mobile = $banner_mobile;
        $blogCategory->alias = request('alias');
        $blogCategory->gallery_id = request('gallery_id');
        $blogCategory->doc = $doc;
        $blogCategory->save();
