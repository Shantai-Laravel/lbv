<template>
    <div>
        <div class="sizeContainer">
          <div v-if="product.subproducts.length" data-toggle="modal" data-target="#modalSize">{{ trans.vars.DetailsProductSet.sizeGuide }}</div>

          <div class="chooseSize animated">
            <label :for="'colSize' + product.id" class="collSize" v-for="subproduct in product.subproducts">
              <input type="radio" :name="'colSize' + product.id" :ref="'colSize' + product.id + subproduct.id" @click="setProduct(product, subproduct)"/>
              <span :class="['check', 'my' + product.id]">{{ subproduct.parameter_value.translation.name }}</span>
            </label>
          </div>
            ...
            <span> {{ btnWish }}
              ...
              >
                <g id="Symbols" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                  <g
                    id="wishButton"
                    transform="translate(-1447.000000, -245.000000)"
                    fill="#42261D"
                    fill-rule="nonzero"
                  >
                    <g id="header">
    ...
    methods: {
        ...
        addToCartProduct(product) {
            if (product.subproducts.length > 0) {
                if (this.subproduct.warehouse.stoc > 0) {
